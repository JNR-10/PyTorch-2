...
def main():
    ...
    if not os.path.exists(data_dir):
        ...
        print("=> Please create a 'data/' directory with training images organized in subfolders.")
        print("=> Example structure: data/images/img1.jpg, data/images/img2.jpg, etc.")
        return
    
    try:
        dataset = MyImageFolder(root_dir=data_dir)
        if len(dataset) == 0:
            print("=> No training data found in the data directory.")
            return
...
if __name__ == "__main__":
    ...
    if try_model:
        print("=> DEBUG: Entering try_model section")
        # Will just use pretrained weights and run on images
        # in test_images/ and save the ones to SR in saved/
        gen = Generator(in_channels=3).to(config.DEVICE)
        opt_gen = optim.Adam(gen.parameters(), lr=config.LEARNING_RATE, betas=(0.0, 0.9))
        
        print(f"=> Attempting to load checkpoint from: {config.CHECKPOINT_GEN}")
        print(f"=> File exists: {os.path.exists(config.CHECKPOINT_GEN)}")
        ...
        if os.path.exists(config.CHECKPOINT_GEN):
            try:
                ...
                print(f"=> Manual load successful. Type: {type(checkpoint)}")
        ...
        if checkpoint_loaded:
            print("=> Checkpoint loaded successfully! Processing test images...")
        else:
            print("=> Failed to load checkpoint. Processing with random weights...")
            print("=> Note: Output quality will be poor without trained weights.")
